...
class FakeCentral {
  ...
  async simulateAdvertisementReceived(scanResult) {
    ...
    let parsedScanResult = JSON.parse(JSON.stringify(scanResult));

    if ('uuids' in scanResult.scanRecord) {
      parsedScanResult.scanRecord.uuids =
    ...
    parsedScanResult.scanRecord.appearance = {
    ...
    parsedScanResult.scanRecord.txPower = {
    ...
    if ('manufacturerData' in scanResult.scanRecord) {
      parsedScanResult.scanRecord.manufacturerData =
    ...
    if ('serviceData' in scanResult.scanRecord) {
      parsedScanResult.scanRecord.serviceData.serviceData = convertToMojoMap(
          scanResult.scanRecord.serviceData, BluetoothUUID.getService);
    }

    await this.fake_central_ptr_.simulateAdvertisementReceived(
        new bluetooth.mojom.ScanResult(parsedScanResult));

    return this.fetchOrCreatePeripheral_(parsedScanResult.deviceAddress);
  }
